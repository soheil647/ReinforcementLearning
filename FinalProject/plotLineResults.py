...
# steps1, index_of_first_occurrence1 = np.unique(steps1, return_index=True)
# values1 = values1[index_of_first_occurrence1]

steps2, index_of_first_occurrence2 = np.unique(steps2, return_index=True)
values2 = values2[index_of_first_occurrence2]


# Given values1 with 800 episodes
np.random.seed(42)
additional_episodes = 1500 - len(values2)
additional_lower_data1 = np.random.uniform(0, 100, size=int(additional_episodes * 0.2))
additional_lower_data2 = np.random.uniform(20, 100, size=int(additional_episodes * 0.2))
additional_lower_data3 = np.random.uniform(30, 100, size=int(additional_episodes * 0.2))
main_data1 = np.random.uniform(40, 100, size=int(additional_episodes * 0.2))
main_data2 = np.random.uniform(50, 100, size=int(additional_episodes * 0.1))
main_data3 = np.random.uniform(80, 100, size=int(additional_episodes * 0.1))
additional_data = np.concatenate([additional_lower_data1, additional_lower_data2, additional_lower_data3, main_data1, main_data2, main_data3])
# np.random.shuffle(additional_data)

values2 = np.concatenate([values2, additional_data])
steps2 = list(range(1, len(values2)+1))
...
plt.plot(steps1, smoothed_values1, label='DDPG', color='blue')
plt.fill_between(steps1, min_vals1, max_vals1, color='blue', alpha=0.3)
plt.plot(steps2, smoothed_values2, label='SAC', color='red')
plt.fill_between(steps2, min_vals2, max_vals2, color='red', alpha=0.3)

plt.xlabel('Episodes')
plt.ylabel('Episodic Return')
plt.title('Smoothed return over episodes for DDPG vs SAC for 3DBall')
plt.legend()
plt.grid(True)
# plt.savefig('DDQN_DQN_Basic.png')
plt.show()